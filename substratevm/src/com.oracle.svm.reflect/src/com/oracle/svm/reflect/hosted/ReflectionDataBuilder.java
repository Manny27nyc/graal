    ...
    private static void registerTypesForAnnotationValue(DuringAnalysisAccessImpl access, Class<?> type, Object value) {
        if (type.isAnnotation() && !SubstitutionReflectivityFilter.shouldExclude(type, access.getMetaAccess(), access.getUniverse())) {
            ...
            try {
                ...
                /*
                 * Trying to access a non-public interface with a class loader that doesn't define
                 * it causes proxy class registration to fail. We ignore this annotation when it
                 * happens.
                 */
                System.out.println("Warning: unable to create proxy class for annotation type " + type + ". Reason: " + e.getMessage());
            }
            ...
            for (Map.Entry<String, Class<?>> entry : annotationType.memberTypes().entrySet()) {
                ...
                try {
                    ...
                    // Ignore the value
                    System.out.println("Warning: unable to register annotation value " + valueName + " for annotation type " + type + ". Reason: " + e);
                }
            }
